...
class PermutedMNISTTask(Task):
    ...
    def __init__(self, seed):
        super().__init__(seed)
        self._permutations = list(permutations(range(10)))
        # Shuffle the permutations so we see examples where all output classes
        # are remapped.
        r = random.Random(seed)
        r.shuffle(self._permutations)
    ...
    def get_dataset(self, i, type=TRAIN) -> Dataset:
        ...
        """
        if type == TRAIN:
            return PermutedMNISTExample(self._permutations[i])
        else:
            return PermutedMNISTExample(self._permutations[int(TRAIN_RATIO * len(self._permutations)):][i])
...
class PermutedMNISTExample(Example):
    def __init__(self, permutation_map):
        self._permutation_map = permutation_map
        transform = transforms.Compose([transforms.ToTensor(), transforms.Normalize((0.5,), (0.5,))])
        self._dataset = torchvision.datasets.MNIST(root='./data', train=True, transform=transform, download=True)
...
def evaluate(task, subject_model_io):
    ...
    for model_idx in range(len(metadata)):
        ...
        example = task.get_dataset(metadata[model_idx]['index'])
...
if __name__ == '__main__':
    ...
    if args.evaluate_subject_model:
        ...
        state_space = [AdamTrainer(task, 100, 10000, weight_decay=0.0001, lr=0.001, device='cuda')]
